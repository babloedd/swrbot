"""
SWR Pro Bot — Supply / Weakness / Resistance (Commodities Edition for Railway)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Зоны:
...
Товары (Forex/CFD):
  💰 XAUUSD — Золото (Gold)
  🥈 XAGUSD — Серебро (Silver)
  🛢️ WTIUSD — Нефть WTI (Oil)
  🔥 XNGUSD — Природный газ (Natural Gas)
  🌾 XCUUSD — Медь (Copper) — в тренде 2026 (AI, электрификация)
  ☕ XCOUSD — Какао (Cocoa) — дефицит 2025-2026
  💵 EURUSD — Евро/Доллар
  💷 GBPUSD — Фунт/Доллар
  💴 USDJPY — Доллар/Йена
"""
...
# === ТОВАРЫ И ВАЛЮТЫ (Forex/CFD символы) ===
# Формат зависит от брокера! Проверь через ccxt:
#   import ccxt; ex = ccxt.oanda(); print(list(ex.load_markets().keys())[:50])
SYMBOLS = [
    "XAU/USD",   # Золото
    "XAG/USD",   # Серебро
    "USOIL/USD", # Нефть WTI
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout)],  # Railway читает stdout
)
logger = logging.getLogger("swr_bot")

# ══════════════════════════════════════════════════════
#  DATA LAYER (Railway: Oanda/Forex.com + fallbacks)
# ══════════════════════════════════════════════════════

# Кэш бирж — создаём один раз
_exchanges = {}

async def _get_exchange(name: str):
    """Получает или создаёт экземпляр биржи."""
    if name not in _exchanges:
        if name == "oanda":
            _exchanges[name] = ccxt.oanda({"enableRateLimit": True})
        elif name == "forexcom":
            _exchanges[name] = ccxt.forexcom({"enableRateLimit": True})
        elif name == "kraken":
...
async def fetch_ohlcv(symbol: str, timeframe: str, limit: int = CANDLE_LIMIT) -> pd.DataFrame:
    """
    Загружает OHLCV для товаров/валют с fallback между брокерами.
    Railway: stdout-логи для отладки.
    """
    # Пробуем брокеров по очереди
    brokers = ["oanda", "forexcom", "kraken", "kucoin"]
    ...
    for broker in brokers:
        try:
            ...
            logger.info("✅ %s: loaded %s %s from %s", symbol, timeframe, len(df), broker)
            return df
        except Exception as exc:
            err_msg = str(exc)
            # Фильтруем "Symbol not found" — не логируем как ошибку
            if "symbol" in err_msg.lower() or "market" in err_msg.lower():
                logger.debug("%s not found on %s", symbol, broker)
            else:
                logger.warning("%s failed on %s: %s", symbol, broker, err_msg[:100])
            continue

    logger.error("❌ All brokers failed for %s %s", symbol, timeframe)
    return pd.DataFrame()


async def close_exchanges():
    """Закрывает все биржевые соединения при shutdown."""
    for name, ex in _exchanges.items():
        try:
            await ex.close()
            logger.info("Closed %s exchange", name)
...
# ══════════════════════════════════════════════════════
#  RAILWAY SHUTDOWN HANDLER
# ══════════════════════════════════════════════════════

async def shutdown(signal_obj, app):
    """Graceful shutdown для Railway."""
    logger.info("Received exit signal %s...", signal_obj.name)
...
def main() -> None:
    ...
    # Railway: polling с правильными параметрами
    app.run_polling(
        drop_pending_updates=True,
        allowed_updates=Update.ALL_TYPES,
        close_loop=False,  # Важно для Railway
    )
