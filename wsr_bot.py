...
SWING_ORDER    = 5
ZONE_TOLERANCE = 0.006
ZONE_HEIGHT    = 0.004

# Сопоставление тикеров для Yahoo Finance
YF_MAP = {
    "XAU/USD": "GC=F",      # Gold Futures
    "XAG/USD": "SI=F",      # Silver Futures
    "USOIL/USD": "CL=F",    # Crude Oil Futures
    "XNG/USD": "NG=F",      # Natural Gas Futures
    "XCU/USD": "HG=F",      # Copper Futures
    "XCO/USD": "CC=F",      # Cocoa Futures
    "EUR/USD": "EURUSD=X",  # EUR/USD
    "GBP/USD": "GBPUSD=X",  # GBP/USD
    "USD/JPY": "USDJPY=X",  # USD/JPY
}

# Сопоставление тикеров для прямого API Bybit v5
BYBIT_MAP = {
    "XAU/USD": "XAUUSD",
    "XAG/USD": "XAGUSD",
    "USOIL/USD": "USOIL",
...
# ══════════════════════════════════════════════════════
#  DATA LAYER (Multi-tier fetching)
# ══════════════════════════════════════════════════════
...
async def _get_exchange(name: str):
    if name not in _exchanges:
        if name == "bybit":
            _exchanges[name] = ccxt.bybit({"enableRateLimit": True})
        elif name == "kraken":
            _exchanges[name] = ccxt.kraken({"enableRateLimit": True})
        elif name == "kucoin":
            _exchanges[name] = ccxt.kucoin({"enableRateLimit": True})
    return _exchanges[name]


def _fetch_bybit_direct_sync(symbol: str, timeframe: str, limit: int) -> pd.DataFrame:
    """Прямой и стабильный запрос к публичному API Bybit V5 (без ограничений хостинга)."""
    bybit_symbol = BYBIT_MAP.get(symbol)
    if not bybit_symbol:
        return pd.DataFrame()
    ...
    url = f"https://api.bybit.com/v5/market/kline?category=linear&symbol={bybit_symbol}&interval={interval}&limit={limit}"
    ...
    ret = data.get("result", {})
    list_data = ret.get("list", [])
    if not list_data:
        return pd.DataFrame()

    parsed = []
    # Bybit возвращает свечи в порядке убывания (новые в начале), переворачиваем список
    ...
    df = df.sort_index()  # В порядке возрастания
    return df


def _fetch_yf_sync(ticker: str, interval: str, range_val: str) -> pd.DataFrame:
    """Синхронный защищенный запрос к API Yahoo Finance с обходом блокировок."""
    ...
    last_err = None
    for host in hosts:
        # Добавление параметров обхода сигнатуры для виджетов
        url = f"https://{host}/v8/finance/chart/{ticker}?interval={interval}&range={range_val}&corsDomain=finance.yahoo.com&.tsrc=finance"
        req = urllib.request.Request(url, headers=headers)
        try:
            with urllib.request.urlopen(req, timeout=10, context=ctx) as response:
                data = json.loads(response.read().decode())
            ...
            last_err = e
            logger.warning("YF host %s failed for %s: %s", host, ticker, e)
            continue
...
async def fetch_ohlcv(symbol: str, timeframe: str, limit: int = CANDLE_LIMIT) -> pd.DataFrame:
    """Загружает свечи, последовательно применяя методы повышенной стабильности."""
    
    # ЭТАП 1: Для доступных активов используем прямой публичный API Bybit (сверхнадежно на хостингах)
    if symbol in BYBIT_MAP:
        try:
            df = await asyncio.to_thread(_fetch_bybit_direct_sync, symbol, timeframe, limit)
            if not df.empty:
                logger.info("✅ %s: loaded %s %s from Bybit API", symbol, timeframe, len(df))
                return df
        except Exception as bybit_err:
            logger.warning("Direct Bybit API failed for %s: %s", symbol, bybit_err)

    # ЭТАП 2: Запросы к Yahoo Finance с маскировкой под виджет
    if symbol in YF_MAP:
        ticker = YF_MAP[symbol]
        try:
            if timeframe == "1h":
                interval, range_val = "1h", "15d"
            elif timeframe == "4h":
                interval, range_val = "1h", "60d"
            elif timeframe == "1d":
                interval, range_val = "1d", "365d"
            else:
                interval, range_val = "1h", "30d"

            df = await asyncio.to_thread(_fetch_yf_sync, ticker, interval, range_val)
            if not df.empty:
                if timeframe == "4h":
                ...
                df = df.tail(limit)
                logger.info("✅ %s: loaded %s %s from Yahoo Finance", symbol, timeframe, len(df))
                return df
        except Exception as yf_err:
            logger.warning("Yahoo Finance failed for %s: %s", symbol, yf_err)

    # ЭТАП 3: Резервный опрос CCXT бирж
    brokers = ["bybit", "kraken", "kucoin"]
    for broker in brokers:
        try:
            exchange = await _get_exchange(broker)
            ccxt_sym = symbol.replace("/", "") if broker == "bybit" else symbol
            if broker == "kraken" and symbol == "XAU/USD":
                ccxt_sym = "XAUt/USD"
            
            raw = await exchange.fetch_ohlcv(ccxt_sym, timeframe, limit=limit)
            if raw:
                df = pd.DataFrame(raw, columns=["ts", "open", "high", "low", "close", "volume"])
                df["ts"] = pd.to_datetime(df["ts"], unit="ms")
                df.set_index("ts", inplace=True)
                logger.info("✅ %s: loaded %s %s from %s fallback", symbol, timeframe, len(df), broker)
                return df
        except Exception as exc:
            logger.debug("%s failed on %s: %s", symbol, broker, str(exc)[:80])
            continue

    logger.error("❌ All data sources failed to fetch OHLCV for %s %s", symbol, timeframe)
    return pd.DataFrame()
...
async def close_exchanges():
    for name, ex in _exchanges.items():
        try:
            await ex.close()
            logger.info("Closed %s exchange connection", name)
        except Exception as e:
...
def get_trend(df: pd.DataFrame) -> str:
    if "ema200" not in df or df["ema200"].isna().sum() > 50:
        return "neutral"
...
def compute_liquidity_levels(df: pd.DataFrame) -> dict:
    ...
    hi_idx = _swing_indices(df["high"], 3, "high")
    ...
    buy_liq  = find_equal_levels(hi_vals)
    sell_liq = find_equal_levels(lo_vals)
    current = float(df["close"].iloc[-1])
    ...
    return {
        "buy_liq":  [l for l in buy_liq  if l["price"] > current],
        "sell_liq": [l for l in sell_liq if l["price"] < current],
    }
...
def _fmt(price: float) -> str:
    if price >= 1000:
        ...
        return "{:,.3f}".format(price).replace(",", " ")
    else:
        return "{:,.5f}".format(price).replace(",", " ")
...
def create_chart(df: pd.DataFrame, symbol: str, tf: str,
    ...
    for z in sup_zones:
        ...
        _add_zone_fill(fig, x0=z["x_start"], x1=x_end,
                       y_bot=z["bottom"], y_top=z["top"],
                       fill_rgba="rgba(38,166,154,{:.2f})".format(alpha),
                       line_color=GRN)
        fig.add_shape(type="line", x0=z["x_start"], x1=x_end,
                      y0=z["price"], y1=z["price"],
                      line=dict(color=GRN, width=1, dash="dot"), row=1, col=1)
        fig.add_annotation(x=x_end, y=z["price"],
                           text="S  {}".format(_fmt(z["price"])),
                           font=dict(color=GRN, size=9, family="monospace"),
                           showarrow=False, xanchor="left", xshift=5, row=1, col=1)
        if z["signal"]:
            fig.add_trace(go.Scatter(
                x=[z["signal"]["x"]], y=[z["signal"]["y"] * 0.9988],
                mode="markers",
                marker=dict(symbol="triangle-up", size=13, color=GRN,
        ...
        if z["confirms"]:
            fig.add_trace(go.Scatter(
                ...
                mode="markers",
                marker=dict(symbol="circle", size=9, color=GRN,
    ...
    for z in res_zones:
        ...
        if z["signal"]:
            fig.add_trace(go.Scatter(
                x=[z["signal"]["x"]], y=[z["signal"]["y"] * 1.0012],
                mode="markers",
        ...
        if z["confirms"]:
            fig.add_trace(go.Scatter(
                ...
                mode="markers",
                marker=dict(symbol="circle", size=9, color=RED,
                            line=dict(color="#ffffff", width=0.8)),
    ...
    fig.add_hline(y=current, line=dict(color="rgba(255,255,255,0.3)", width=1, dash="dot"), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(
        ...
        showlegend=False, hovertemplate="EMA200: %{y:,.0f}<extra></extra>"), row=1, col=1)
    
    if not df["ema200"].isna().all():
        fig.add_annotation(x=x_end, y=float(df["ema200"].iloc[-1]), text="EMA200",
                           font=dict(color=TREND_COLOR.get(trend, "rgba(150,150,150,0.5)"), size=9, family="monospace"),
                           showarrow=False, xanchor="left", xshift=5, row=1, col=1)

    # LIQUIDITY
    for lvl in liq_levels.get("buy_liq", []):
        lc = "rgba(255,213,79,0.9)" if lvl["swept"] else "rgba(255,213,79,0.5)"
        ds = "solid" if lvl["swept"] else "dot"
        fig.add_hline(y=lvl["price"], line=dict(color=lc, width=1, dash=ds), row=1, col=1)
        lbl = "LIQ ×{} {}".format(lvl["count"], "✓swept" if lvl["swept"] else "")
        fig.add_annotation(x=df.index[int(len(df)*0.02)], y=lvl["price"], text=lbl,
                           font=dict(color="rgba(255,213,79,0.8)", size=8, family="monospace"),
                           showarrow=False, xanchor="left", yshift=6, row=1, col=1)
    ...
    for lvl in liq_levels.get("sell_liq", []):
        ...
        fig.add_annotation(x=df.index[int(len(df)*0.02)], y=lvl["price"], text=lbl,
                           font=dict(color="rgba(255,213,79,0.8)", size=8, family="monospace"),
                           showarrow=False, xanchor="left", yshift=-10, row=1, col=1)

    # RSI
    fig.add_trace(go.Scatter(
        x=df.index, y=df["rsi"], name="RSI",
        line=dict(color="#7b61ff", width=1.5),
        fill="tozeroy", fillcolor="rgba(123,97,255,0.06)"), row=2, col=1)
    for lvl, col in [(70, RED), (50, "#444"), (30, GRN)]:
        fig.add_hline(y=lvl, line=dict(color=col, width=1, dash="dash"), opacity=0.5, row=2, col=1)
    ...
    fig.add_trace(go.Bar(x=df.index, y=df["macd_hist"],
                         marker_color=bar_colors, opacity=0.75, showlegend=False), row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["macd"],
                             line=dict(color="#2962ff", width=1.5), showlegend=False), row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["macd_signal"],
                             line=dict(color="#ff6d00", width=1.5), showlegend=False), row=3, col=1)
    ...
    fig.update_layout(
        ...
        title=dict(text="<b>{}</b>  ·  {}  ·  SWR Pro".format(symbol, tf),
...
def generate_signal_text(df: pd.DataFrame, symbol: str, tf: str,
    ...
    if dist_sup < 0.010 and rsi < 45 and macd > macd_s and long_allowed:
        emoji, signal = ("🟢", "ЛОНГ  —  зона поддержки + sweep ликвидности снизу") if swept_below else ("🟢", "ЛОНГ  —  цена у зоны поддержки, тренд подтверждает")
    elif dist_res < 0.010 and rsi > 55 and macd < macd_s and short_allowed:
        emoji, signal = ("🔴", "ШОРТ  —  зона сопротивления + sweep ликвидности сверху") if swept_above else ("🔴", "ШОРТ  —  цена у зоны сопротивления, тренд подтверждает")
...
def kb_main(symbol: str, tf: str) -> InlineKeyboardMarkup:
    emojis = {
        "XAU/USD": "💰", "XAG/USD": "🥈", "USOIL/USD": "🛢️",
        "XNG/USD": "🔥", "XCU/USD": "🌾", "XCO/USD": "☕",
        "EUR/USD": "💵", "GBP/USD": "💷", "USD/JPY": "💴",
    }

    def sym_btn(s: str) -> InlineKeyboardButton:
        emoji = emojis.get(s, "📊")
        label = ("✓ " if s == symbol else "") + emoji + " " + s.replace("/USD", "")
    ...
    return InlineKeyboardMarkup([
        ...
        [sym_btn(s) for s in SYMBOLS[:3]],
        [sym_btn(s) for s in SYMBOLS[3:6]],
        [sym_btn(s) for s in SYMBOLS[6:]],
        [
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "*Привет, {}!*\n\n".format(name) +
        "Я — *SWR Pro Bot* (Commodities Edition)\n\n"
        "💰 Золото (XAU)  ·  🥈 Серебро (XAG)\n"
        "🛢️ Нефть WTI  ·  🔥 Газ (NG)\n"
        "🌾 Медь (Copper)  ·  ☕ Какао\n"
        "💵 EUR/USD  ·  💷 GBP/USD  ·  💴 USD/JPY\n\n"
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "*SWR Pro Bot — справка (Commodities)*\n\n"
        "/start  — главное меню\n"
...
async def _do_chart(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    if df.empty:
        await msg.edit_text("❌ Не удалось загрузить данные. Проверь символ или попробуй позже.")
        return
    ...
    try:
        # Выполнение отрисовки графика в фоновом потоке
        img = await asyncio.to_thread(create_chart, df, symbol, tf, sup_zones, res_zones, liq_levels)
        current = float(df["close"].iloc[-1])
...
async def cmd_alerts(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    current = ctx.user_data.get("alerts_on", False)
    ...
    if ctx.user_data["alerts_on"]:
        lines_text = [
            ...
            "",
            "Мониторю: Золото, Серебро, Нефть, Газ, Медь, Какао, EUR/USD, GBP/USD, USD/JPY",
            "Таймфреймы: 1h, 4h, 1d",
            "",
...
ALERT_SCHEDULE: dict = {"1h": 900, "4h": 1800, "1d": 3600}
...
def _alert_key(symbol: str, tf: str, side: str, zone_price: float) -> str:
    if zone_price >= 1000:
        rounded = round(zone_price, -1)
    elif zone_price >= 100:
        rounded = round(zone_price, -1)
    elif zone_price >= 1:
        rounded = round(zone_price, 1)
    else:
        rounded = round(zone_price, 3)
...
def _check_fresh_signal(df: pd.DataFrame, sup_zones: list, res_zones: list) -> list:
    ...
    for z in res_zones:
        if hi >= z["bottom"] and cl < z["bottom"] and cl < op:
            signals.append({"side": "short", "zone_price": z["price"],
                            "candle_price": hi, "zone_top": z["top"], "zone_bot": z["bottom"]})

    for z in sup_zones:
        if lo <= z["top"] and cl > z["top"] and cl > op:
            signals.append({"side": "long", "zone_price": z["price"],
                            "candle_price": lo, "zone_top": z["top"], "zone_bot": z["bottom"]})
    return signals
            ...
            for symbol in ALERT_SYMBOLS:
                try:
                    ...
                    for user_id, user_data in app.user_data.items():
                        if not user_data.get("alerts_on", False):
                            continue
...
async def post_shutdown(app: Application) -> None:
    """Освобождение ресурсов при остановке контейнера."""
    logger.info("Initiating post-shutdown procedures...")
...
def main() -> None:
    logger.info("🚀 Starting SWR Pro Bot (Commodities Edition) on Railway...")
